...
def generate_halton_reference():
    ...
    for i, sim_val in enumerate(sim_values):
        ...
        print(
            f"k={k}: Sim=[{sim_val[0]:6d}, {sim_val[1]:6d}] Exp=[{expected[0]:6d}, {expected[1]:6d}] {status}"
        )
    ...
    match_reseed = (
        sim_reseed[0] == expected_reseed[0] and sim_reseed[1] == expected_reseed[1]
    )
    status_reseed = "PASS" if match_reseed else "FAIL"
    print(
        f"Sim=[{sim_reseed[0]:6d}, {sim_reseed[1]:6d}] Exp=[{expected_reseed[0]:6d}, {expected_reseed[1]:6d}] {status_reseed}"
    )
